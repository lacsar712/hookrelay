...
func New(clk clock.Clock, broker *queue.Broker, dests *destination.Registry, gates *runtime.Gates, client *deliver.Client, jlog *journal.Log, dead *dlq.Queue, policy backoff.Policy) *Engine {
	return &Engine{
		clk:    clk,
		broker: broker,
		dests:  dests,
...
func (e *Engine) handle(ctx context.Context, j job.Job) {
	...
	if wait > 0 {
		...
		e.broker.Enqueue(j, dest.Ordered, dest.MaxInFlight)
		e.note(j, "rate_limited", 0, "", "rate_limited")
		return
	}

	j.Attempt++
	res := e.client.Post(ctx, deliver.Request{
		URL:           dest.URL,
		Secret:        dest.Secret,
		EventID:       j.EventID,
