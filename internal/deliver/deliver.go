...
func (c *Client) Post(ctx context.Context, req Request) Result {
	...
	httpReq.Header.Set("X-Hook-Timestamp", fmt.Sprintf("%d", unix))
	httpReq.Header.Set("X-Hook-Nonce", nonce)
	httpReq.Header.Set("X-Hook-Signature", sig)
	httpReq.Header.Set("X-Hook-Event-Id", req.EventID)
	httpReq.Header.Set("X-Hook-Delivery-Id", req.DeliveryID)
	httpReq.Header.Set("X-Hook-Attempt", fmt.Sprintf("%d", req.Attempt))
	httpReq.Header.Set("X-Hook-Destination", req.DestinationID)
	httpReq.Header.Set("X-Hook-Body-Sha256", hashutil.SHA256Hex(req.Body))
